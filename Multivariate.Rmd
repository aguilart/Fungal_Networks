...
knitr::opts_chunk$set(fig.height = 15, fig.width = 10)
```
...
saveRDS(Edge_Traits,"List_Edge_Traits.RDS")
saveRDS(Node_Traits,"List_Node_Traits.RDS")
#Edge_Traits<-do.call(rbind,myfiles)
```
...
Edge_Traits$Species[grep("C41",Edge_Traits$name)]<-"Fusarium redolens"
Edge_Traits$Species[grep("FOX",Edge_Traits$name)]<-"Fusarium oxysporum"
Edge_Traits$Species[grep("DF32",Edge_Traits$name)]<-"Fusarium solani"

Edge_Traits$Species[grep("Pi",Edge_Traits$name)]<-"Pi"
Edge_Traits$Species[grep("Pv",Edge_Traits$name)]<-"Pv"
Edge_Traits$Species[grep("Rb",Edge_Traits$name)]<-"Rb"
...
Node_Traits$Species[grep("Pi",Node_Traits$name)]<-"Pi"
Node_Traits$Species[grep("Pv",Node_Traits$name)]<-"Pv"
Node_Traits$Species[grep("Rb",Node_Traits$name)]<-"Rb"
...
Hist_Width_Length<-
        Re_org%>%
        filter(variable %in% c("Width","Length"))%>%#1_The completely indepedent variables
...
Edge_Traits%>%
        ...
        filter(variable %in% c("Log_Length"))%>%#4_Tortuosity has little variation and depends on Length
        ggplot()+
        aes(value,fill=name)+
        geom_histogram()+
        facet_wrap(Species ~ variable, scales = "free",nrow = 6,ncol = 2)+
...
Checking width distribution of tips
...
Edge_Traits%>%
        ...
        filter(variable %in% c("Width"))%>%#4_Tortuosity has little variation and depends on Length
        ggplot()+
        aes(value,fill=name)+
        geom_histogram()+
        facet_wrap(Species ~ variable, scales = "free",nrow = 6,ncol = 2)+
        theme(legend.position = "none")
```
This means that tip width can be log transformed

```{r}
tapply(log10(Edge_Traits[-which(Edge_Traits$EndNodes_1_Degree==1|Edge_Traits$EndNodes_1_Degree==2),]$Width),
       Edge_Traits[-which(Edge_Traits$EndNodes_1_Degree==1|Edge_Traits$EndNodes_1_Degree==2),]$name,
       summary)
```
...
Edge_Traits%>%
        #filter(!(EndNodes_1_Degree==1|EndNodes_2_Degree==1))%>%
  filter(EndNodes_1_Degree==3|EndNodes_2_Degree==3)%>%
        select(c("name","Species","Width","Length","Area","Volume","Resistance_2ave","Tortuosity","Distance",
                 "Accessibility","Betweenness","Route_factor","Or_ij"))%>%
        mutate(Log_Length=log10(Length))%>%
        group_by(name)%>%
...
Node_Traits%>%
  filter(node_Type=="E") %>% 
        ggplot()+
        aes(node_Omin_Omid,fill=name)+
        geom_histogram()+
        facet_wrap(.~Species, scales = "free",nrow = 6,ncol = 2)+
        theme(legend.position = "none")
```

```{r}
tapply(Node_Traits$node_Omin_Omid,Node_Traits$name,summary,na.rm=T)
```
...
Edge_Traits%>%
        ...
        #filter(variable %in% c("Width"))%>%#4_Tortuosity has little variation and depends on Length
        ggplot()+
        aes(as.factor(EndNodes_1_Degree),Width)+
...
Node_Traits_c<-Node_Traits
Node_Traits_c$Species<-NA
Node_Traits_c$Species[grep("C34",Node_Traits_c$name)]<-"Mortierella elongata"
Node_Traits_c$Species[grep("C35",Node_Traits_c$name)]<-"Umbelopsis isabellina"
Node_Traits_c$Species[grep("DF19",Node_Traits_c$name)]<-"Mortierella alpina"
Node_Traits_c$Species[grep("DF25",Node_Traits_c$name)]<-"Mortierella elongata2"
Node_Traits_c$Species[grep("DF56",Node_Traits_c$name)]<-"Mucor fragilis"
Node_Traits_c$Species[grep("M",Node_Traits_c$name)]<-"Mortierella alpina2"

Node_Traits_c$Species[grep("DF9",Node_Traits_c$name)]<-"Alternaria sp"
Node_Traits_c$Species[grep("C41",Node_Traits_c$name)]<-"Fusarium redolens"
Node_Traits_c$Species[grep("FOX",Node_Traits_c$name)]<-"Fusarium oxysporum"
Node_Traits_c$Species[grep("DF32",Node_Traits_c$name)]<-"Fusarium solani"

Node_Traits_c$Species[grep("Pi",Node_Traits_c$name)]<-"Pi"
Node_Traits_c$Species[grep("Pv",Node_Traits_c$name)]<-"Pv"
Node_Traits_c$Species[grep("Rb",Node_Traits_c$name)]<-"Rb"
...
Re_org_2<-
  ...
  filter(node_Degree==1)%>%
        ...
        mutate(Route_Length=node_Distance*node_RF)%>%
        select(c("name","Species","node_Distance","node_RF","node_Accessibility","Route_Length"))%>%
        group_by(name)%>%
        gather(key=variable,
               value=value,node_Distance:Route_Length)
names(Edge_Traits)
```
...
```{r,fig.height = 15, fig.width = 10}
        ...
        #filter(variable %in% c("Distance"))%>%#1_The completely indepedent variables
        ggplot()+
        aes(Route_Length,node_Accessibility)+
        geom_point()+
...
```{r}
        Re_org_2%>%
          ...
          filter(variable %in% c("node_Accessibility"))%>%#1_The completely indepedent variables
          
        ggplot()+
        #aes(log10(value),fill=name)+
        aes(log10(value),fill=name)+
        geom_histogram()+
        facet_wrap(Species ~ variable, scales = "free",nrow = 4,ncol = 3)+
        theme(legend.position = "none")
```
...
```{r}
        Re_org_2%>%
  filter(EndNodes_1_Degree==1|EndNodes_2_Degree==1)%>%
        filter(variable %in% c("Route_factor"))%>%#1_The completely indepedent variables
        ggplot()+
        aes(value,fill=name)+
        geom_histogram()+
        facet_wrap(Species ~ variable, scales = "free",nrow = 6,ncol = 4)+
...
```{r}
        Re_org_2%>%
  filter(EndNodes_1_Degree==1|EndNodes_2_Degree==1)%>%
        filter(variable %in% c("Route_Length"))%>%#1_The completely indepedent variables
        ggplot()+
        aes(value,fill=name)+
        geom_histogram()+
        facet_wrap(Species ~ variable, scales = "free",nrow = 6,ncol = 4)+
...
```{r,echo=FALSE,eval=FALSE}
rmarkdown::render("Multivariate.Rmd", quiet = T)
```
