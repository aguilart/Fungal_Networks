...
Basidios1%>% 
  ...
  geom_text(aes(label=easy_code,fontface="bold.italic"),size=1,color="darkgreen") +
    labs(y="RDA2",x="RDA1")+
...
Ascos_zygos1%>% 
  ...
  geom_text(size=1,aes(x=RDA1_species/2.5,y=RDA2_species/2.5,label=traits,fontface="bold"),
            hjust = 1.1, vjust =-0.5,#segment.size = 0,segment.color = 'transparent',
...
all_data_t1<-all_data_t
all_data_t1$Root_eff_l_scaled<-NULL
all_data_t1$Reff_tip_l_scaled<-NULL
all_data_t1$Root_eff_a_scaled<-all_data_t1$Root_eff/all_data_t1$Mycelial_area
all_data_t1$Reff_tip_a_scaled<-all_data_t1$Reff_tip/all_data_t1$Mycelial_area
all_data_t1$Root_eff<-NULL
all_data_t1$Reff_tip<-NULL
all_data_t1$Mycelial_area<-NULL

all_data_t2<-all_data_t
all_data_t2$Root_eff<-NULL
all_data_t2$Reff_tip<-NULL
all_data_t2$Mycelial_area<-NULL

all_data_t_centered1<-sapply(all_data_t1,function(x){(x-mean(x))/sd(x)})
all_data_t_centered2<-sapply(all_data_t2,function(x){(x-mean(x))/sd(x)})
...
model_triangle1<-rda(all_data_t1,scale=T)
...
summary(model_triangle2)[["cont"]][["importance"]][,c(1:2)]
```
