...
temporal1<-cbind(temporal1,all_data_scaled[,c("name_col")])
...
Saving the location of the archetypes as obtained through Uri Alon´s software
```{r,eval=FALSE}
#triangle points

#archetypes based on scaled data only
d1<-data.frame(x=c(5.686,-7.451,0.118),y=c(-3.476,-2.545,5.321))
d1<-d1/4.7 #I divided by this number because the scaling of the Uri Alon´s software PCA is different from the one PCA scaling of vegan

```


Plotting the PCA with the archetypes and triangle
```{r,eval=FALSE}
...
temporal1%>% 
  ...
  labs(x="PCA 1 (33.38%)",y="PCA 2 (26.78%)")+
  #xlim(-1.1,1)+
  #ylim(-1.2,1.3)+
  my_theme#+
...
Which is exactly the same as the one produced in the Uri Alon´software (produced on May 1st 2021 in my Hp laptop):
![](triangle_2May.png)
...
The issue is that wtih the new data from Mark on basidiomycetes and mine coming from the most updated app (the last one of April 2021), this triangle is not significant. This test was based on 10,000 permutations. The output I get for the triangle above: 

How many repetitions would you like to perform? 10000
Checking P-Value with 10000 tries
Pvalue is:  0.163183681632
...
```{r,eval=FALSE}
#Rb_ctrl1_d8_1
ecol<-E(colonies_ntwk[[36]])$width
ecol[E(colonies_ntwk[[36]])[type=="Inoculum"]]<-(quantile(ecol[E(colonies_ntwk[[36]])[type!="Inoculum"]])[2])/2
#ecol2<-cscale(ecol,palette = seq_gradient_pal(low="#F7FBFF", high = "#08306B"))
ecol2<-cscale(ecol,palette = div_gradient_pal(low = "#FDE725FF",mid = "#21908CFF",high = "#440154FF"))
ecol2[E(colonies_ntwk[[36]])[type=="Inoculum"]]<-"gray30"

jpeg("Updated_Figures\\Rb_ctrl1_d8_1_for_triangle.jpg",width = 961,height = 703)
par(mar=c(0,0,0,0))
plot(colonies_ntwk[[36]],
     edge.arrow.size=1,edge.curved=0,
     edge.width=2,
     #edge.width=ecol/3,
     vertex.label=NA,vertex.shape="none",
     edge.color=ecol2,
     edge.size=150,vertex.size=0,layout=spatial.data[[36]]*1,main="Rb_ctrl1_d8_1")
dev.off()
...
#10.Pi Phallus impudicus Pi_ctrl1_d8_1
ecol<-E(colonies_ntwk[[33]])$width
ecol[E(colonies_ntwk[[33]])[type=="Inoculum"]]<-(quantile(ecol[E(colonies_ntwk[[33]])[type!="Inoculum"]])[2])/2
#ecol2<-cscale(ecol,palette = seq_gradient_pal(low="#F7FBFF", high = "#08306B"))
ecol2<-cscale(ecol,palette = div_gradient_pal(low = "#FDE725FF",mid = "#21908CFF",high = "#440154FF"))
ecol2[E(colonies_ntwk[[33]])[type=="Inoculum"]]<-"gray30"
