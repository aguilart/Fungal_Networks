...
summaryTraits_Mark_AZ<-readRDS("summaryTraits_Mark_AZ.RDS")
...
Basidios1%>% 
  ...
  geom_text(aes(label=easy_code,fontface="bold.italic"),size=1,color="#0C2C84") +
    labs(y="RDA2",x="RDA1")+
  
  geom_text(size=1,aes(x=RDA1_species/2,y=RDA2_species/2,label=traits,fontface="bold"),
...
Ascos_zygos1%>% 
  ...
  geom_text(aes(color=phylum,label=easy_code),size=1) +
    labs(y="RDA2",x="RDA1")+
  
  geom_text(size=1,aes(x=RDA1_species,y=RDA2_species,label=traits,fontface="bold"),
            hjust = 1.1, vjust =-0.5,#segment.size = 0,segment.color = 'transparent',
...
all_data_scaled1%>% 
  ...
  aes(x=RDA1,y=RDA2) +
  #geom_point(size=2)+
    geom_text(aes(label=easy_code,fontface="bold.italic",color=phylum),size=1) +
    labs(y="RDA2",x="RDA1")+
...
all_data_t<-
all_data_scaled[,c(#"Hyphal_length",
                                                     "Hyphal_number",
                                                     #"Hyphal_tip_width",
                                                     #"Hyphal_main_width",
                                                     "Hyphal_angle",                                                                              #"Mycelial_area",
...
all_data_t_centered<-sapply(all_data_t,function(x){(x-mean(x))/sd(x)})

write.csv2(all_data_t_centered,"alldata_forTriangularity0.csv",row.names = F)#This one can ber read from the hp desktop at Uni

write.csv(all_data_t_centered,"alldata_forTriangularity.csv",row.names = F)#This one can be read in my laptop
rm(all_data_t_centered)

model_triangle<-rda(all_data_t,scale=T)
#Adding the two RDA axis to the dataframe
temporal<-as.data.frame(scores(model_triangle,display = "sites",scaling = "sites",choices=c(1,2)))

write.csv(temporal,"axes_triangularity.csv",row.names = F)
...
all_data_scaled_t<-cbind(all_data_scaled,temporal)#;rm(temporal)

e_vectors_triangle<-as.data.frame(
  scores(model_triangle,display = "species",choices = c(1,2),scaling = "sites"));
e_vectors_triangle$traits<-rownames(e_vectors_triangle)
names(e_vectors_triangle)[1]<-"PC1_s"
names(e_vectors_triangle)[2]<-"PC2_s"

summary(model_triangle)[["cont"]][["importance"]][,c(1:2)]
```
...
d<-data.frame(x=c(-0.881,0.839,0.592),y=c(0.057,1.043,-0.914))
...
all_data_scaled_t%>% 
  ...
  aes(x=PC1,y=PC2) +
  geom_point(size=2)+
  geom_polygon(data=d,aes(x=x,y=y),alpha=0.50,fill="#21908CFF")+
  ...
  #geom_text_repel(aes(label=name_col,fontface="bold.italic",color=phylum),size=3) +
      labs(x="PCA 1 (37.92%)",y="PCA 2 (25.85%)")+
      my_theme
